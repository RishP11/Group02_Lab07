...
int main(void)
{
    ...
    while(1){
        ...
        unsigned char received_byte = UART_Rx();
        // Process received data and control LEDs
        if (UART7_RSR_R != 0x00){
            GPIO_PORTF_DATA_R = 0x02;  // Light up RED LED for error
        }
        else if (received_byte == 0xAA){
            GPIO_PORTF_DATA_R = 0x08;  // Light up GREEN LED
        }
        else if (received_byte == 0xF0){
            GPIO_PORTF_DATA_R = 0x04;  // Light up BLUE LED
        }
    }
}
...
void UART_setup( void )
{
    ...
    BRD -= BRDI ;                                               // Extracting the fractional part of BRD
    ...
    UART7_LCRH_R = 0x62 ;                                       // Line Control (Serial Parameters) of the UART
    UART7_CC_R = 0x00 ;                                         // Clock source of the register
    UART7_CTL_R |= (1 << 9) | (1 << 8) | (1 << 0) ;             // Enabling the UART
}

void PORT_E_init( void )
{
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGC2_GPIOE ;                  // Enable clock to GPIO_E
